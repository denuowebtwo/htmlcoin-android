    ...
    public static Observable<String> getPropertyValue(final String propName, final Contract contract, final Context context) {
        ...
        return Observable.fromCallable(new Callable<Pair<String[], ContractMethod>>() {
            ...
            public Observable<String> call(final Pair<String[], ContractMethod> pair) {
                return QtumService.newInstance().callSmartContractInfo(contract.getContractAddress(), Joiner.on(",").join(pair.first), contract.getSenderAddress())
                        .map(new Func1<CallSmartContractResponse, String>() {
                            @Override
                            public String call(CallSmartContractResponse callSmartContractResponse) {
                                return processResponse(pair.second.getOutputParams(), callSmartContractResponse.getExecutionResult().getOutput());
                            }
                        });
            }
        });
    ...
    public static Observable<String> getPropertyValue(final Contract contract, final ContractMethod contractMethod) {
        ...
        return Observable.fromCallable(new Callable<String[]>() {
            ...
            public Observable<String> call(String[] strings) {
                return QtumService.newInstance().callSmartContractInfo(contract.getContractAddress(), Joiner.on(",").join(strings), contract.getSenderAddress())
                        .map(new Func1<CallSmartContractResponse, String>() {
                            @Override
                            public String call(CallSmartContractResponse callSmartContractResponse) {
                                return processResponse(contractMethod.getOutputParams(), callSmartContractResponse.getExecutionResult().getOutput());
                            }
                        });
            }
        });
